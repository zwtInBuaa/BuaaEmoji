...
class Flowers(ImageFolder):
    def __init__(self, root, train=True, transform=None, **kwargs):
        ...
        for item in self.imgs:
            self.samples.append(
                (os.path.join(root, 'jpg', "image_{:05d}.jpg".format(item)), self.img_to_label[item - 1] - 1))
...
class BuaaEmoji(ImageFolder):
    def __init__(self, root, train=True, transform=None, target_transform=None, **kwargs):
        self.dataset_root = root
        self.loader = default_loader
        self.target_transform = None
        self.transform = transform
        train_list_path = os.path.join(self.dataset_root, 'Emoji', 'train.txt')
        test_list_path = os.path.join(self.dataset_root, 'Emoji', 'test.txt')
